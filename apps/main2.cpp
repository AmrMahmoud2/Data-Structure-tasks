...
double *sum2( double a , double b )
{
    double *p_results = new double (a + b);
    return p_results;
}
...
// Your explanation: we are swapping the variables a and b locally as they are not linked to the input variables (no references nor pointers)
//and as this is a void fun.; it doesn't return the changed values. So the function doesn't output any swapped variables.
void swap3( double a , double b )
{
    double temp_a = a;
    a = b;
